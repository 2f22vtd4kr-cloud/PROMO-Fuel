...
- **Country availability checker**: GET /api/factory/countries?api_key=KEY&service=11 → returns \`{countries:[{id,name,stock,price}], cached:bool, ttl:60}\` sorted cheapest first. Results cached 60s per API key. UI button "📊 Check Stock" next to the Country field — opens an inline panel showing live stock (🟢>50 🟡10-50 🔴<10) and USD price per number; clicking a row auto-selects the country. Shows a search filter. Requires SMSPool API key to be filled in first.
