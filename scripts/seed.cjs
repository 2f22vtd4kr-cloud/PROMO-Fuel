...
const DB_PATH = path.resolve(__dirname, "../campaigns.db");
const db = new Database(DB_PATH);

function daysAgo(n, hoursOffset) {
  const d = new Date();
  d.setDate(d.getDate() - (n || 0));
  d.setHours(hoursOffset || 10, Math.floor(Math.random() * 59), 0, 0);
...
const insAcc = db.prepare(`
  ...
  VALUES (?,?,?,?,?,?,?,?,?,?)
`);
for (const a of accs) insAcc.run(a.label,a.phone,a.username,a.status,a.sent_today,a.sent_total,a.failed_total,a.is_banned,a.is_active,daysAgo(rand(20,50)));
...
const TAG_SETS  = ['["vip","active"]','["premium"]','["active"]','["new"]','["inactive"]','["vip"]','["active","new"]','["premium","active"]','["inactive"]','["active"]'];
const insUser   = db.prepare(`INSERT OR IGNORE INTO users (chat_id,username,first_name,tags,first_seen,last_seen) VALUES (?,?,?,?,?,?)`);
const chatIds   = [];
for (let i = 0; i < 80; i++) {
  const chatId = 100000000 + i * 1337 + rand(1, 50);
  insUser.run(chatId, HANDLES[i % HANDLES.length] + (i > 24 ? i : ""), NAMES[i % NAMES.length], TAG_SETS[i % TAG_SETS.length], daysAgo(rand(5,60)), daysAgo(rand(0,4)));
  chatIds.push(chatId);
}
...
const existingCamps = db.prepare("SELECT COUNT(*) as n FROM campaigns").get().n;
if (existingCamps > 0) {
  console.log(`  → skipped (${existingCamps} already exist)`);
} else {
  const insCamp = db.prepare(`INSERT INTO campaigns (name,text_template,status,created_at,started_at,sent_count,failed_count,target_count) VALUES (?,?,?,?,?,?,?,?)`);
  const camps = [
    ...
    { name:"АИ-98 премиум клиентам",    tmpl:"✨ Только для VIP: АИ-98 по цене АИ-95 сегодня с 09:00 до 18:00.",  status:"done",      ago:2, pct:1.0,  target:300  },
  ...
  for (const c of camps) {
    ...
    insCamp.run(c.name, c.tmpl, c.status, daysAgo(c.ago + 1, 9), c.pct > 0 ? daysAgo(c.ago, 10) : null, sent, Math.round(sent * 0.04), c.target);
  }
...
if (existingSends > 100) {
  ...
  const insertBatch = db.transaction((rows) => { for (const r of rows) insSend.run(r.c, r.u, r.a, r.s, r.t); });
  const rows = [];
  for (const c of camps) {
    const total = Math.min(c.sent_count, 250);
    const failRate = c.failed_count / Math.max(c.sent_count, 1);
    const base = c.started_at ? new Date(c.started_at).getTime() : Date.now();
    for (let i = 0; i < total; i++) {
      rows.push({ c: c.id, u: chatIds[i % chatIds.length], a: accountIds[i % accountIds.length], s: Math.random() < failRate ? "error" : "ok", t: new Date(base + i * 4500).toISOString() });
    }
  }
  insertBatch(rows);
